...
def signal_plot_to_png(signal: np.ndarray, id: int, page: int, one_image_len: int, add_name: str="") -> str:
    ...
    for idx, ax in enumerate(axs):
        ...
        if len(for_fig_size_arr) == 0:
            for_fig_size_arr = np.zeros((len(cut_signal)))
...
def processed_signal_plot_to_png(signal: np.ndarray, id: int, page: int, one_image_len: int, add_name: str="") -> str:
    fig = plt.figure(figsize=(15, 2*6), constrained_layout=True)
    axs = fig.subplots(6, 1)
    for idx, ax in enumerate(axs):
        major_ticks_top=np.arange(0,one_image_len+256,256)
        ...
        qs = np.percentile(cut_signal, [1, 99], interpolation='nearest')
        q_1 = qs[0]
        q_3 = qs[1]
        iqr = q_3 - q_1
        y_min = q_1 - 0.3*iqr

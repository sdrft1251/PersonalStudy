...
if __name__=="__main__":
    ...
    ######################### Model reconstruct #########################
    exp_id = "4"
    runi_id = "1e4a44074a124866a78e2b3b4ce89f5c"
    ### Model Loading
    model_1 = mlflow.keras.load_model(os.path.join("/works/CES2022/mlflow/mlruns", exp_id, runi_id, "artifacts/model"))
    print(model_1.summary())
    train_predict = model_1.predict(train_inputs)
    val_predict = model_1.predict(val_inputs)
    test_predict = model_1.predict(test_inputs)
    train_mse = ((train_inputs - train_predict) ** 2).mean()
    val_mse = ((val_inputs - val_predict) ** 2).mean()
    test_mse = ((test_inputs - test_predict) ** 2).mean()
    print(f"Model 1 -> Train : {train_mse} | Val : {val_mse} | Test : {test_mse}")
    ...
    exp_id = "4"
    runi_id = "b6a6008f22a44762ac7c00637aa0c56b"
    ### Model Loading
    model_2 = mlflow.keras.load_model(os.path.join("/works/CES2022/mlflow/mlruns", exp_id, runi_id, "artifacts/model"))
    print(model_2.summary())
    train_predict = model_2.predict(train_inputs_encoder_1)
    val_predict = model_2.predict(val_inputs_encoder_1)
    test_predict = model_2.predict(test_inputs_encoder_1)
    train_mse = ((train_inputs_encoder_1 - train_predict) ** 2).mean()
    val_mse = ((val_inputs_encoder_1 - val_predict) ** 2).mean()
    test_mse = ((test_inputs_encoder_1 - test_predict) ** 2).mean()
    print(f"Model 2 -> Train : {train_mse} | Val : {val_mse} | Test : {test_mse}")
    ...
    def return_model():
        ...
        x = layer_7(x)
        x = layer_8(x)
        x = tf.keras.layers.Dense(64, name="fin_dense_1")(x)
        x = tf.keras.layers.ReLU(name="fin_relu_1")(x)
        x = tf.keras.layers.Dense(64, name="fin_dense_2")(x)
        x = tf.keras.layers.ReLU(name="fin_relu_2")(x)
        x = tf.keras.layers.Dense(32, name="fin_dense_3")(x)
        x = tf.keras.layers.ReLU(name="fin_relu_3")(x)
        x = tf.keras.layers.Dense(32, name="fin_dense_4")(x)
        x = tf.keras.layers.ReLU(name="fin_relu_4")(x)
        x = tf.keras.layers.Dense(2, activation="softmax", name="fin_dense_5")(x)
    ...
    with mlflow.start_run(run_name="FFT_DNN_220105_0", tags={"sub": "fin", "v": "0.2", "data_source": config["data_source"], "describe": "pre1 + pre2 + Expanding layer"}) as run:

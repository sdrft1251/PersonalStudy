...
def encoder(time_size, dims, hidden_size, num_heads, latent_length, dropout, recurrent_dropout, name="Encoder"):
    ...
    # Init
    encoder_lstm_cell = tf.keras.layers.LSTMCell(units=hidden_size, dropout=dropout, recurrent_dropout=recurrent_dropout, name="Ecoding_LSTM_Cell")
    encoder_rnn_wrapper = tf.keras.layers.RNN(encoder_lstm_cell, return_state=True, name="Encoder_RNN_Wrapper")
    encoder_bidirect_wrapper = tf.keras.layers.Bidirectional(encoder_rnn_wrapper, name="Encoder_Bi_Wrapper")
    # Attention
    attention = attention_module(time_len=time_size, dims=dims, num_heads=num_heads, dropout=dropout)(inputs)
    # Bi-LSTM
    _, h_state_forw, _, h_state_back, _  = encoder_bidirect_wrapper(attention)
    h_state_concat = tf.concat([h_state_forw, h_state_back], axis=1, name="Encoder_hstate_Concat")
    mu = tf.keras.layers.Dense(latent_length, name="Encoding_MU_Dense")(h_state_concat)
    sigma = tf.keras.layers.Dense(latent_length, name="Encoding_STD_Dense")(h_state_concat)
    sigma = tf.math.softplus(sigma, name="Encoder_std_softplus")
    return tf.keras.Model(inputs=inputs, outputs=[mu, sigma], name=name)

def decoder(latent_length, time_size, num_heads, hidden_size, dropout, recurrent_dropout, name="Decoder"):
    # For Collection
    mu_collect = []
    sigma_collect = []
    # Input
    ...
    # Cell
    decoder_lstm_cell = tf.keras.layers.LSTMCell(units=hidden_size, dropout=dropout, recurrent_dropout=recurrent_dropout, name="Decoding_LSTM_Cell")
    decoder_rnn_wrapper = tf.keras.layers.RNN(decoder_lstm_cell, return_sequences=True, name="Decoder_RNN_Wrapper")
    decoder_bidirect_wrapper = tf.keras.layers.Bidirectional(decoder_rnn_wrapper, name="Decoder_Bi_Wrapper")
    decode_mu_dense = tf.keras.layers.Dense(1, activation="sigmoid", name="Decoding_MU_Dense")
    #decode_std_dense = tf.keras.layers.Dense(1, name="Decoding_STD_Dense")
    # Expand
    outputs = tf.repeat(tf.expand_dims(inputs, axis=1, name="Expand_z_sample"), time_size, axis=1)
    # Attention
    attention = attention_module(time_len=time_size, dims=latent_length, num_heads=num_heads, dropout=dropout)(outputs)
    # Bi-LSTM
    outputs  = decoder_bidirect_wrapper(attention)
    mu = decode_mu_dense(outputs)
    #sigma = decode_std_dense(outputs)
    #sigma = tf.math.softplus(sigma, name="Decoder_std_softplus")
...
def vae(time_size, d_model, num_heads, enc_hidden_size, latent_length, dec_hidden_size, dropout, recurrent_dropout, reparam=True, name="VAE"):
    ...
    num_heads=num_heads, hidden_size=dec_hidden_size, dropout=dropout, recurrent_dropout=recurrent_dropout)(z)

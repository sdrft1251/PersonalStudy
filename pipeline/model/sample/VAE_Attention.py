...
class Vec2Time(tf.keras.layers.Layer):
    ...
    def call(self, input_tensor):
        ...
        v1 = self.v1_dense(tf.math.sign(input_tensor))
        return v0 + v1
...
def encoder_layer(time_len, d_model, num_heads, compress_time_len, compress_dims_len, dropout, name="encoder_layer"):
    ...
    # FFNN
    outputs = tf.keras.layers.Dense(units=int(d_model/2), activation='relu')(attention)
...
dec_layer_num, dec_first_dims, expand_time_lens, expand_dims_lens, dropout, name="transformer"):
    inputs = tf.keras.Input(shape=(time_len,1), name="inputs")
...
def get_lr(warmup_steps, d_model, step):
    ...
    return float(np.reciprocal(np.sqrt([d_model]))[0] * min([arg1, arg2]))

def train(model, train_set, epochs, batch_size, d_model, beta_cycle, beta_rate, summary_dir, add_name, cp_dir):
    train_loss_results = []
    train_metric_results = []
    # Set Optimizer
    custom_sch = CustomSchedule(d_model, warmup_steps=200)
    optimizer = tf.keras.optimizers.Adam(learning_rate=custom_sch)
    # For File Save Name
    ...
    for ep_ in range(epochs):
        ...
        learning_rate_value = get_lr(200, d_model, ep_)

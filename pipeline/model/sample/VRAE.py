...
class ModelPipe(tf.keras.Model):

    def __init__(self, **params):
        super(ModelPipe, self).__init__(name='')

        self.time_size = params['time_size']
        self.hidden_size = params['hidden_size']

        # For Ecoder
        self.encode_cell = tf.keras.layers.SimpleRNNCell(self.hidden_size, dropout=params['dropout'], recurrent_dropout=params['recurrent_dropout'], name="Ecoding_RNN_Cell")
        self.encode_rnn = tf.keras.layers.RNN(self.encode_cell, return_state=True, name="RNN_Wrapper")
        self.encode_mu_dense = tf.keras.layers.Dense(params['latent_length'], name="Encoding_MU_Dense")
        self.encode_std_dense = tf.keras.layers.Dense(params['latent_length'], name="Encoding_STD_Dense")

        # For Decoder
        self.decoder_first_state_dense = tf.keras.layers.Dense(self.hidden_size, activation='tanh', name="Decoding_first_state_Dense")
        self.decode_cell = tf.keras.layers.SimpleRNNCell(self.hidden_size, dropout=params['dropout'], recurrent_dropout=params['recurrent_dropout'], name="Decoding_RNN_Cell")
        self.decoder_output_mu_dense = tf.keras.layers.Dense(params['output_depth'], activation="sigmoid", name="Decoding_mu_Output")
...
##### Train Function
###############################################################################################
def train(model, data_col,  reparam=True, **params):
    # Get Config
    time_size = params['time_size']
    over_len = params['over_len']
    batch_size = params['batch_size']
    beta_cycle = params['beta_cycle']
    beta_rate = params['beta_rate']
    epochs = params['epochs']
    learning_rate = params['learning_rate']
    summary_dir = params['summary_dir']
    add_name = params['add_name']
    cp_dir = params['cp_dir']

...
class FetchingData(BaseOperator):
    ...
    ####################
    ##### 특정 Org와 Site에 속하는 test 가져오기
    def getdata_from_org_site(self):
        cur = self.conn.cursor(buffered=False)
        metadata_cur = self.metadata_conn.cursor()

        query = sql.q_get_test_from_org_site()
        cur.execute(query)
        ecgtests = cur.fetchall()

        for (test_id, test_seq, rec_start, dur, org_id, site_id) in ecgtests:
            self.save_files(test_id, test_seq, rec_start, dur, org_id, site_id, cur, metadata_cur)



        return -1

    def getdata_from_org_site_seq(self):
        return -1

    def getdata_from_none(self):
        return -1

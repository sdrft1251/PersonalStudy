...
######################## FFT ########################
def fft_trans(data_arr, sampling_rate, lowest_hz, highest_hz):
    fft_trans = abs(np.fft.fft(data_arr, norm="ortho", axis=-1))
    index = np.fft.fftfreq(sampling_rate*10, 1/sampling_rate)
    return fft_trans[:,np.logical_and(index>=lowest_hz, index<=highest_hz)]
######################## Min-Max Scaling ########################
def minmax_scaling(data_arr):
    max_val = data_arr.max(axis=1).reshape(-1,1)
    max_val = np.where(max_val==0.0, 1, max_val)
    return data_arr/max_val
################################################
...
def merging(data_list, label_list, split_type, split_rates=[0.8, 0.1]):
    ...
    for idx, val in enumerate(data_list):
        ...
        val = fft_trans(val, sampling_rate=256, lowest_hz=0.3, highest_hz=50)
...
if __name__ == "__main__":
    ...
    base_path = "/home/wellysis-tft/Desktop/code/datalake/processed_data/CES2022/set_2"

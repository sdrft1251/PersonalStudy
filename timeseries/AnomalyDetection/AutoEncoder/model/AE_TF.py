...
################# Total Model
############################################################################################################################################################
def transformer_vae(time_len, d_model, enc_layer_num, num_heads, dec_layer_num, dropout, name="transformer"):
    inputs = tf.keras.Input(shape=(time_len,1), name="inputs")
    ...
    # Decoder Part
    dec_outputs = decoder(time_len=time_len, num_layers=dec_layer_num, d_model=d_model, num_heads=num_heads, dropout=dropout)(inputs=[inputs, enc_outputs])

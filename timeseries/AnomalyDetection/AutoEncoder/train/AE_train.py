...
def elbo_loss(model, inputs):
    ...
    reconstruct_er = tf.reduce_sum(tf.math.square(inputs-outputs), 1)
    reconstruct_er = tf.reduce_mean(reconstruct_er)
    # For MSE & MAE
...
def grad(model, inputs):
    with tf.GradientTape() as tape:
        reconstruct_er, mse, mae = elbo_loss(model, inputs)
    return reconstruct_er, mse, mae, tape.gradient(reconstruct_er, model.trainable_variables)

def train(model, train_set, epochs, batch_size, learning_rate, summary_dir, add_name, cp_dir, sample_data_set):
    train_loss_results = []
    ...
    for ep_ in range(epochs):
        ...
        for x in train_dataset:
            # Get Grad
            reconstruct_er, mse, mae, grads = grad(model, x)
            # Apply Gradient

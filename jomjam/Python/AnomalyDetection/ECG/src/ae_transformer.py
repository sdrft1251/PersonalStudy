...
def transformer_ae(time_len, d_model, enc_layer_num, num_heads, compress_dims, latent_len, exand_dims, dec_layer_num, dropout, reparam=True, name="transformer"):
    inputs = tf.keras.Input(shape=(time_len,1), name="inputs")
    ...
    # Latent Space
    enc_mu = tf.keras.layers.Dense(units=latent_len)(enc_outputs)

    # Expand Layer
    z = tf.keras.layers.Dense(time_len, name="For_Expand_Dense_2")(enc_mu)
    ...
    return tf.keras.Model(inputs=inputs, outputs=[outputs, enc_mu], name=name)

...
def elbo_loss(model, inputs, bernoulli=True):
    ...
    marginal_likelihood = (-tf.reduce_mean(marginal_likelihood))*100
...
def train(model, data_col, time_size, over_len, batch_size,\
epochs, learning_rate=0.001, summary_dir="/logs", add_name="", cp_dir="/save"):
    ...
    for ep_ in range(epochs):
        ...
        for x in train_dataset:
            elbo, reconstruct_er, kld, mse, grads = grad(model, x)
            # Apply Gradient

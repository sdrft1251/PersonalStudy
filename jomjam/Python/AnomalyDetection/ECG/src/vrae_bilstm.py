...
class VRAE_BILSTM(tf.keras.Model):
    def __init__(self, time_size, enc_hidden_size, latent_length, dec_hidden_size, dropout, recurrent_dropout):
        ...
        self.encoder_bidirect_wrapper = tf.keras.layers.Bidirectional(self.encoder_rnn_wrapper, name="Encoder_Bi_Wrapper")
    ...
    def encoder(self, input_tensor):
        ...
        _, h_state_forw, _, h_state_back, _  = self.encoder_bidirect_wrapper(input_tensor)
        # Concat
        h_state_concat = tf.concat([h_state_forw, h_state_back], axis=1, name="Encoder_hstate_Conat")
        # Get Mean & STD
        mu = self.encode_mu_dense(h_state_concat)
        std = self.encode_std_dense(h_state_concat)
    ...
    def decoder(self, input_tensor):
        ...
        # # Expand dimension
        # input_tensor = tf.repeat(tf.expand_dims(input_tensor, axis=1, name="Expand_z_sample"), self.time_size, axis=1)
        # # Pass Bi-LSTM
        # output_seq, _, _, _, _ = self.decoder_bidirect_wrapper(input_tensor)
        # # Get Output
        # mu = self.decode_mu_dense(output_seq)
        # std = self.decode_std_dense(output_seq)
        # std = tf.math.softplus(std, name="Decoder_std_softplus")
        # return mu, std
...
def train(model, numpy_dir, time_size, batch_size, beta_cycle, beta_rate, reparam, epochs,\
learning_rate, summary_dir, add_name, cp_dir):
    ...
    for ep_ in range(epochs):
        ...
        train_set = train_set[np.random.permutation(len(train_set))[:5000]]

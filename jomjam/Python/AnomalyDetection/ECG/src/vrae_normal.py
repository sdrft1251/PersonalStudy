...
class VRAE(tf.keras.Model):
    ...
    def __init__(self, time_size, enc_hidden_size, latent_length, dec_hidden_size, dropout, recurrent_dropout):
        ...
        # For Ecoder
        self.encode_cell = tf.keras.layers.SimpleRNNCell(units=enc_hidden_size, dropout=dropout, recurrent_dropout=recurrent_dropout, name="Ecoding_RNN_Cell")
        ...
        self.decode_cell = tf.keras.layers.SimpleRNNCell(units=dec_hidden_size, dropout=dropout, recurrent_dropout=recurrent_dropout, name="Decoding_RNN_Cell")
        self.decode_mu_dense = tf.keras.layers.Dense(1, name="Decoding_mu_Output")
        self.decode_std_dense = tf.keras.layers.Dense(1, name="Decoding_std_Output")
    ...
    def encoder(self, input_tensor):
        ...
        # get mean & std
        mu = self.encode_mu_dense(last_states)
        std = self.encode_std_dense(last_states)
    ...
    def decoder(self, input_tensor):
        ...
        for t in range(self.time_size):
            ...
            # Next Cell
            rnn_out, h_state = self.decode_cell(inputs=input_tensor, states=h_state)
            # Collect

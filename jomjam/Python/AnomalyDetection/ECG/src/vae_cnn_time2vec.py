...
def vae_cnn_time2vec(time_len, name="vae_cnn_time2vec"):
    inputs = tf.keras.Input(shape=(time_len,1), name="inputs")
    # Embedding Part
    embeddings = Time2Vec(output_dims=512)(inputs)
    # Encoder Part
    enc = tf.keras.layers.Conv1D(512,5, activation=tf.keras.layers.LeakyReLU())(embeddings)
    enc = tf.keras.layers.Conv1D(256,3, activation=tf.keras.layers.LeakyReLU())(enc)
    enc = tf.keras.layers.Conv1D(128,3, activation=tf.keras.layers.LeakyReLU())(enc)
    enc = tf.keras.layers.Conv1D(64,3, activation=tf.keras.layers.LeakyReLU())(enc)
    enc = tf.keras.layers.Conv1D(32,3, activation=tf.keras.layers.LeakyReLU())(enc)
    enc = tf.keras.layers.Flatten()(enc)

    mu_enc = tf.keras.layers.Dense(32)(enc)
    sigma_enc = tf.keras.layers.Dense(32)(enc)
    sigma_enc = tf.math.softplus(sigma_enc)
    # Latent Space
    z = mu_enc + sigma_enc * tf.random.normal(tf.shape(mu_enc), 0, 1, dtype=tf.float32)
    z = tf.keras.layers.Reshape((32,1))(z)
    z = tf.keras.layers.Dense(64)(z)
    # Decoder Part
    # 64,64
    dec_1 = tf.keras.layers.Conv1D(64, 3, padding="same", activation=tf.keras.layers.LeakyReLU())(z)
    dec_2 = tf.keras.layers.Conv1D(64, 3, padding="same", activation=tf.keras.layers.LeakyReLU())(z)
    dec = tf.keras.layers.concatenate([dec_1, dec_2], axis=1)
    # 128, 128
    dec_1 = tf.keras.layers.Conv1D(128, 3, padding="same", activation=tf.keras.layers.LeakyReLU())(dec)
    dec_2 = tf.keras.layers.Conv1D(128, 3, padding="same", activation=tf.keras.layers.LeakyReLU())(dec)
    dec = tf.keras.layers.concatenate([dec_1, dec_2], axis=1)

    # 256, 256
    dec_1 = tf.keras.layers.Conv1D(256, 3, padding="same", activation=tf.keras.layers.LeakyReLU())(dec)
    dec_2 = tf.keras.layers.Conv1D(256, 3, padding="same", activation=tf.keras.layers.LeakyReLU())(dec)
    dec = tf.keras.layers.concatenate([dec_1, dec_2], axis=1)
    ...
    # For Output
    outputs = tf.keras.layers.Dense(1, activation='sigmoid')(dec)
    return tf.keras.Model(inputs=inputs, outputs=[outputs, mu_enc, sigma_enc], name=name)

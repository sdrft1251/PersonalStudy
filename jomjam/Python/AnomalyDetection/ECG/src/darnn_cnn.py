...
# Model Part
####################################################################################
def encoder(T, d_model, hidden_size, h0=None, name="encoder"):
    inputs = tf.keras.Input(shape=(T,1), name="inputs")
    # Compress
    compress = Conv1D(filters=d_model, kernel_size=5, strides=3)(inputs)
    compress = Conv1D(filters=d_model, kernel_size=4)(compress)
    compress = Conv1D(filters=d_model, kernel_size=4)(compress)
    compress = ReLU()(compress)
    compress = Conv1D(filters=d_model, kernel_size=5, strides=3)(compress)
    compress = Conv1D(filters=d_model, kernel_size=4)(compress)
    compress = Conv1D(filters=d_model, kernel_size=4)(compress)
    compress = ReLU()(compress) # batch, compressT, d_model
    # Encoder
...
def DARNN(T, d_model, batch_size, hidden_size, latent_dims, name="DARNN"):
    ...
    # Model Part
    inputs = tf.keras.Input(shape=(T,1), name="inputs")
    enc_output = encoder(T=T, d_model=d_model, hidden_size=hidden_size, h0=h0)(inputs) # batch, compressT, d_model

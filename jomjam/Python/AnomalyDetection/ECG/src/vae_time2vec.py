...
def vae_time2vec(time_len, d_model, hidden_size, latent_length, dropout, recurrent_dropout, reparam=True, name="vae_time2vec"):
    inputs = tf.keras.Input(shape=(time_len,1), name="inputs")

    # Encoder Part
    embeddings = Time2Vec(output_dims=d_model)(inputs)

    encode_cell = tf.keras.layers.LSTMCell(hidden_size, dropout=dropout, recurrent_dropout=recurrent_dropout, name="Ecoding_LSTM_Cell")
    encode_rnn = tf.keras.layers.RNN(encode_cell, return_state=True, name="Encoding_RNN_Wrapper")
    encode_mu_dense = tf.keras.layers.Dense(latent_length, name="Encoding_MU_Dense")
    encode_std_dense = tf.keras.layers.Dense(latent_length, name="Encoding_STD_Dense")

    # Encoder Part
    _, last_states, _ = encode_rnn(embeddings)

    # get mean & std
    mu_enc = encode_mu_dense(last_states)
    sigma_enc = encode_std_dense(last_states)
    ...
    # Expand
    z = tf.repeat(tf.expand_dims(z, axis=1), repeats=time_len, axis=1)

    decode_cell = tf.keras.layers.LSTMCell(hidden_size, dropout=dropout, recurrent_dropout=recurrent_dropout, name="Decoding_LSTM_Cell")
    decode_rnn = tf.keras.layers.RNN(decode_cell, return_sequences=True, return_state=True, name="Decoding_RNN_Wrapper")
    decode_out_dense = tf.keras.layers.Dense(1, activation="sigmoid", name="For_Output")

    # Decoder Part
    whole_seq_output, final_memory_state, final_carry_state = decode_rnn(z)

    outputs = decode_out_dense(whole_seq_output)

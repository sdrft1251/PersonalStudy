...
def encoder(time_len, num_layers, compress_nums, d_model, num_heads, dropout, name="encoder"):
    # layer nums check -> 해당 개수 만큼 끝에 설정
    assert compress_nums<=num_layers
    # 초기값 설정
    comp_start = num_layers-compress_nums
    # Model Part
    ...
    for i in range(num_layers):
        ...
        if i >= comp_start:
            ...
            outputs = tf.keras.layers.Conv1D(filters=d_model, kernel_size=1, padding='same', name="CompressLayer_dim_{}_1".format(i))(outputs)
            outputs = tf.keras.layers.Conv1D(filters=d_model, kernel_size=1, padding='same', name="CompressLayer_dim_{}_2".format(i))(outputs)
            outputs = tf.keras.layers.LeakyReLU()(outputs)
            outputs = tf.keras.layers.MaxPooling1D(pool_size=2, strides=2, padding='same', name="CompressLayer_time_{}".format(i))(outputs)
    return tf.keras.Model(inputs=inputs, outputs=outputs, name=name)
...
################# Total Model
############################################################################################################################################################
def transformer_vae(time_len, d_model, enc_layer_num, compress_nums, num_heads, dec_layer_num, dropout, name="transformer"):
    inputs = tf.keras.Input(shape=(time_len,1), name="inputs")

    # Encoder Part
    enc_outputs = encoder(time_len=time_len, num_layers=enc_layer_num, compress_nums=compress_nums, d_model=d_model, num_heads=num_heads, dropout=dropout)(inputs=inputs)

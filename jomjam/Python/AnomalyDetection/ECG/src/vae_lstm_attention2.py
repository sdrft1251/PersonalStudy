...
class auto_module(tf.keras.layers.Layer):
    def __init__(self, time_size, hidden_size, dropout, recurrent_dropout, name="auto_module"):
        ...
        self.decoder_lstm_cell = tf.keras.layers.LSTMCell(units=hidden_size, dropout=dropout, recurrent_dropout=recurrent_dropout, name="Decoding_LSTM_Cell")
    ...
    def call(self, input_tensor):
        ...
        for i in range(self.time_size):
            ...
            # Next Cell
            _, states = self.decoder_lstm_cell(inputs=x_out, states=(h_state, c_state))
            h_state = states[0]
            c_state = states[1]
            # Collect
...
def encoder(time_size, dims, hidden_size, num_heads, latent_length, dropout, recurrent_dropout, name="Encoder"):
    ...
    # LSTM
    _, h_state, _ = encoder_rnn_wrapper(attention)
    mu = tf.keras.layers.Dense(latent_length, name="Encoding_MU_Dense")(h_state)
    sigma = tf.keras.layers.Dense(latent_length, name="Encoding_STD_Dense")(h_state)
